...
with open(output_txt, "w") as log:
    for img_file in image_files:
        ...
        for i, det in enumerate(detections):
            ...
            if decoded:
                dm_text = decoded[0].data.decode('utf-8').strip().replace(" ", "_")
                print(f"[✓] {img_file} → {dm_text}")
                log.write(f"{img_file} -> {dm_text}\n")
                filename = f"Success_{img_file.split('.')[0]}_DM{dm_text}.png"
                success = True
            else:
                dm_text = f"DM{i+1}"
                filename = f"notDetected_{img_file.split('.')[0]}_{dm_text}.png"

            # === Save crop ===
            out_path = os.path.join(crop_dir, filename)
            cv2.imwrite(out_path, enhanced)  # save enhanced grayscale
